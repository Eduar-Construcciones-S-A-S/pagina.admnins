...
interface Plan {
  id_plan: number;
  nombre_plan: string;
  codigo_plan?: string | null; // legado: ya no se usa para la operación CH
...
export default function PlanesAdmin(){
  ...
  const fetchData=async()=>{
    try{
      const [p,c]=await Promise.all([getPlanes(),getCodigosOperativos()]);
      setPlanes(p); setCodigos(c);
    }catch(e){console.error(e);}finally{setLoading(false);}
  };
  useEffect(()=>{fetchData();},[]);
  useEffect(()=>{
    if(!supabase)return;
    const channel=supabase.channel("planes-admin")
      .on("postgres_changes",{event:"*",schema:"public",table:"plan"},fetchData)
      .on("postgres_changes",{event:"*",schema:"public",table:"codigo_operativo"},fetchData)
      .subscribe();
  ...
  const codigosPorPlan=useMemo(()=>{
    const map=new Map<number,CodigoOperativo[]>();
    for(const c of codigos){if(c.id_plan==null)continue;if(!map.has(c.id_plan))map.set(c.id_plan,[]);map.get(c.id_plan)!.push(c);}
    for(const [,list] of map)list.sort((a,b)=>a.codigo_ch.localeCompare(b.codigo_ch));
    return map;
  },[codigos]);
  ...
  const handleSave=async()=>{
    ...
    catch(e){console.error(e);alert("Ocurrió un error al guardar el plan.");}finally{setSaving(false);}
  };
  ...
  return <div className="planes-page">
    ...
    <div className="planes-kpis">
      <div className="kpi-card line-blue"><div className="kpi-icon" style={{background:"#dbeafe",color:"#1e40af"}}><Package size={24}/></div><div className="kpi-info"><h3>{planes.length}</h3><p>Total planes</p></div></div>
      <div className="kpi-card line-green"><div className="kpi-icon" style={{background:"#ecfdf5",color:"#0f766e"}}><Package size={24}/></div><div className="kpi-info"><h3>{planesConCodigo}</h3><p>Planes con CH · {codigosVinculados} códigos vinculados</p></div></div>
    </div>
