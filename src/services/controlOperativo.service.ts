...
export type ControlOperativoRow = {
  ...
  id_plan:number|null; id_hora:number|null; plan:string; fecha:string; hora:string; aprobado:boolean|null; nombre:string; edad:number|null; nacionalidad:string;
...
export async function getControlOperativo():Promise<ControlOperativoRow[]>{
  ...
  for(const r of reservas as any[]){
    ...
    for(const p of personas){const contactoCliente=text(p?.telefono_cliente||r.telefono_cliente);rows.push({
      ...
      id_plan:r.id_plan==null?null:Number(r.id_plan),id_hora:horaId==null?null:Number(horaId),plan:text(plan?.nombre_plan),fecha:fechaReserva,hora:horaReserva,aprobado:r.aprobado??null,
