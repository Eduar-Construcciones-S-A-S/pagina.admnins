...
export async function getControlOperativo(): Promise<ControlOperativoRow[]> {
  ...
  for (const f of fechas as any[]) {
    const possibleIds = [f.id_fecha, f.id_plan_fecha, f.id];
    for (const id of possibleIds) if (id != null) fechaMap.set(Number(id), f);
  }
  for (const h of horas as any[]) {
    const possibleIds = [h.id_hora, h.id_plan_hora, h.id];
    for (const id of possibleIds) if (id != null) horaMap.set(Number(id), h);
  }
  ...
  for (const r of reservas as any[]) {
    ...
    const horaId = r.id_hora ?? r.id_plan_hora ?? r.id_hora_reserva;
    const fechaRelacionada = fechaMap.get(Number(fechaId));
    const horaRelacionada = horaMap.get(Number(horaId));
    const personas = participantesPorReserva.get(Number(r.id_reserva)) ?? [null];

    const fechaReserva = dateOnly(
      r.fecha_reserva ??
      r.fecha ??
      fechaRelacionada?.fecha_reserva ??
      fechaRelacionada?.fecha
    );
    const horaReserva = hourOnly(
      r.hora_reserva ??
      r.hora ??
      horaRelacionada?.hora_reserva ??
      horaRelacionada?.hora
    );
    ...
    const totalRespaldo = num(r.valor_total) > 0 ? num(r.valor_total) : num(r.precio_unitario) * cantidadPersonas;
...
export async function updateControlReserva(idReserva: number, payload: Record<string, unknown>) {
  const { data, error } = await client().from("reserva").update(payload).eq("id_reserva", idReserva).select().single();
  if (error) throw error;
  return data;
}

export async function updateControlParticipante(idParticipante: number, payload: Record<string, unknown>) {
  const { data, error } = await client().from("participante").update(payload).eq("id_participante", idParticipante).select().single();
  if (error) throw error;
  return data;
}
