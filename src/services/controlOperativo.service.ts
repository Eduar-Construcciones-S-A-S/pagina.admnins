...
export type ControlOperativoRow = {
  id_reserva: number; id_participante: number | null; reserva_codigo: string; id_plan: number | null; id_hora: number | null;
  plan: string; fecha: string; hora: string; aprobado: boolean | null; nombre: string; edad: number | null; nacionalidad: string;
  tipo_documento: string; documento: string; contacto: string; contacto_cliente: string; cantidad: number | null; mina: boolean | null;
  refrigerio: boolean | null; restaurante: string; almuerzo: string; total: number; abono: number; medio_abono: string;
  pago_saldo: number; medio_saldo: string; saldo_pendiente: number; observacion: string;
};
...
export async function getControlOperativo(): Promise<ControlOperativoRow[]> {
  const [reservasRes, participantesRes, planesRes, fechasRes, horasRes] = await Promise.all([
    ...
    client().from("participante").select("*").order("id_participante", { ascending: true }), client().from("plan").select("*"),
    client().from("plan_fechas").select("*"), client().from("plan_horas").select("*"),
  ]);
  ...
  for(const r of reservas as any[]){
    ...
    for(const p of personas){const contactoCliente=text(p?.telefono_cliente||r.telefono_cliente);rows.push({id_reserva:Number(r.id_reserva),id_participante:p?Number(p.id_participante):null,reserva_codigo:text(r.codigo_reserva)||`#${r.id_reserva}`,id_plan:r.id_plan==null?null:Number(r.id_plan),id_hora:horaId==null?null:Number(horaId),plan:text(plan?.nombre_plan),fecha:fechaReserva,hora:horaReserva,aprobado:r.aprobado??null,nombre:text(p?.nombre),edad:p?.edad==null?null:Number(p.edad),nacionalidad:text(p?.nacionalidad),tipo_documento:text(p?.tipo_documento),documento:text(p?.numero_documento),contacto:text(p?.telefono_participante||contactoCliente),contacto_cliente:contactoCliente,cantidad:cantidadPersonas,mina:r.mina??null,refrigerio:r.refrigerio??null,restaurante:text(r.restaurante),almuerzo:text(p?.tipo_almuerzo),total,abono,medio_abono:text(r.metodo_pago_abono),pago_saldo:pagoSaldo,medio_saldo:text(r.metodo_pago_saldo),saldo_pendiente:saldo,observacion:text(r.observacion)});}
  }
  return rows;
}
