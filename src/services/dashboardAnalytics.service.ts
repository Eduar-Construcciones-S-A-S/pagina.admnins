...
export async function getDashboardAnalytics(): Promise<DashboardAnalyticsData> {
  ...
  return { reservas:(reservasResult.data??[]) as unknown as DashboardReserva[], planes:(planesResult.data??[]) as DashboardPlan[], clientes:(clientesResult.data??[]) as DashboardCliente[], participantes:(participantesResult.data??[]) as DashboardParticipante[] };
}
