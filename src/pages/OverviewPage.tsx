...
function reservationDate(r: DashboardReserva) {
  return dateKey(r.fecha_reserva || r.fecha_aprobacion || r.fecha_solicitud);
}
...
export default function OverviewPage() {
  ...
  const metrics = useMemo(() => {
    ...
    const buildRanking = (reservas: DashboardReserva[]) => {
      const map = new Map<number,{id:number;nombre:string;reservas:number;personas:number;ingresos:number}>();
      for (const p of planes) map.set(p.id_plan,{id:p.id_plan,nombre:p.nombre_plan,reservas:0,personas:0,ingresos:0});
      for (const r of reservas) {
        if (!r.id_plan) continue;
        const item=map.get(r.id_plan)||{id:r.id_plan,nombre:planName(r,planes),reservas:0,personas:0,ingresos:0};
        item.reservas+=1; item.personas+=Number(r.cantidad_personas||0); item.ingresos+=totalReserva(r,planes); map.set(r.id_plan,item);
      }
      const total = reservas.reduce((s,r)=>s+totalReserva(r,planes),0);
      return [...map.values()].map(p=>({...p,ticket:p.reservas?p.ingresos/p.reservas:0,participacion:total?p.ingresos/total*100:0})).sort((a,b)=>b.ingresos-a.ingresos||b.reservas-a.reservas);
    };
    ...
    for (const r of aprobadas) {
      const key=reservationDate(r); if(!key) continue;
      const item=dayMap.get(key)||{key,ventas:0,personas:0,ingresos:0};
      item.ventas+=1; item.personas+=Number(r.cantidad_personas||0); item.ingresos+=totalReserva(r,planes); dayMap.set(key,item);
    }
    ...
    const monthly=Array.from({length:6},(_,index)=>{
      ...
      const rows=allApproved.filter(r=>monthKey(reservationDate(r))===key);
      return { key, label:monthLabel(d), ventas:rows.length, personas:rows.reduce((s,r)=>s+Number(r.cantidad_personas||0),0), ingresos:rows.reduce((s,r)=>s+totalReserva(r,planes),0) };
    });
    ...
    const thisMonthRevenue=allApproved.filter(r=>{const d=reservationDate(r);return d>=thisFrom&&d<=thisTo;}).reduce((s,r)=>s+totalReserva(r,planes),0);
    const prevMonthRevenue=allApproved.filter(r=>{const d=reservationDate(r);return d>=prevFrom&&d<=prevTo;}).reduce((s,r)=>s+totalReserva(r,planes),0);
  ...
  const maxPlan=Math.max(1,...metrics.ranking.map(p=>p.ingresos));
  const maxHistoricalPlan=Math.max(1,...metrics.historicalRanking.map(p=>p.ingresos));
  ...
  return <div className="crm-dashboard">
    ...
    <section className="crm-filter-card">
      <div className="crm-filter-title"><CalendarDays size={17}/><div><strong>Periodo personalizado</strong><span>Deja las fechas vacías para ver toda la información. Selecciona un rango solo cuando necesites analizar días específicos.</span></div></div>
      <div className="crm-filter-grid">
        <label><span>Desde</span><input type="date" value={fromDate} max={toDate||undefined} onChange={e=>setFromDate(e.target.value)}/></label>
        <label><span>Hasta</span><input type="date" value={toDate} min={fromDate||undefined} onChange={e=>setToDate(e.target.value)}/></label>
        <button className="crm-filter-clear" onClick={clearDateFilter} disabled={!dateFilterActive}><X size={14}/> Limpiar fechas</button>
      </div>
      <div className="crm-filter-result">Mostrando <b>{filteredReservas.length}</b> reservas {dateFilterActive?`· ${fromDate||"inicio"} a ${toDate||"hoy"}`:"· todo el histórico"}</div>
    </section>

    <div className="crm-kpis"><Kpi icon={<CircleDollarSign size={20}/>} label="Ventas aprobadas" value={money(metrics.ventasTotal)} helper={`${metrics.aprobadas} reservas`} tone="gold"/><Kpi icon={<Banknote size={20}/>} label="Total recaudado" value={money(metrics.totalCobrado)} helper={metrics.ventasTotal?`${pct(metrics.totalCobrado/metrics.ventasTotal*100)} recaudado`:"Sin ventas"} tone="green"/><Kpi icon={<WalletCards size={20}/>} label="Cartera pendiente" value={money(metrics.cartera)} helper="Saldo por cobrar" tone="red"/><Kpi icon={<Percent size={20}/>} label="Conversión" value={pct(metrics.conversion)} helper={`${metrics.aprobadas} aprobadas · ${metrics.pendientes} pendientes`} tone="blue"/><Kpi icon={<TrendingUp size={20}/>} label="Ticket promedio" value={money(metrics.ticketPromedio)} helper="Por reserva aprobada" tone="violet"/><Kpi icon={<UserCheck size={20}/>} label="Personas vendidas" value={metrics.ocupacionVendida.toLocaleString("es-CO")} helper="Según periodo visible" tone="teal"/></div>
