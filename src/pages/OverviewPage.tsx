...
export default function OverviewPage() {
  ...
  const metrics = useMemo(() => {
    ...
    for (const r of aprobadas) {
      const key=dateKey(r.fecha_aprobacion||r.fecha_solicitud); if(!key) continue;
    ...
    const monthly=Array.from({length:6},(_,index)=>{
      ...
      const rows=allApproved.filter(r=>monthKey(r.fecha_aprobacion||r.fecha_solicitud)===key);
