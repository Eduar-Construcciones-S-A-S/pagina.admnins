...
const hourKey = (value: string) => value ? value.slice(0, 5) : "";
const normalizeHour = (value: string) => hourKey(value) || "—";
...
function exportExcel(rows: ControlOperativoRow[]) {
  ...
  a.href = url; a.download = `control-operativo-${new Date().toISOString().slice(0, 10)}.xls`; a.click();
...
export default function ControlOperativoPage() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    setError(null);
    try {
      const [operativo, pagos, restaurantesData] = await Promise.all([getControlOperativo(), getMetodosPagoActivos(), getRestaurantesActivos()]);
      setRows(operativo);
  ...
  const planes = useMemo(() => [...new Set(rows.map(r => r.plan).filter(Boolean))].sort((a,b) => a.localeCompare(b, "es")), [rows]);
  const fechas = useMemo(() => [...new Set(rows.map(r => dateKey(r.fecha)).filter(Boolean))].sort().reverse(), [rows]);
  const horas = useMemo(() => [...new Set(rows.map(r => hourKey(r.hora)).filter(Boolean))].sort(), [rows]);
  const restaurantesFiltro = useMemo(() => [...new Set([...restaurantes, ...rows.map(r => r.restaurante).filter(Boolean)])].sort((a,b) => a.localeCompare(b, "es")), [rows, restaurantes]);
  const metodosFiltro = useMemo(() => [...new Set([...metodosPago, ...rows.flatMap(r => [r.medio_abono, r.medio_saldo]).filter(Boolean)])].sort((a,b) => a.localeCompare(b, "es")), [rows, metodosPago]);
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r => {
      if (fecha && dateKey(r.fecha) !== fecha) return false;
      if (plan && normalizeText(r.plan) !== normalizeText(plan)) return false;
      if (hora && hourKey(r.hora) !== hora) return false;
      if (mina === "si" && r.mina !== true) return false;
      if (mina === "no" && r.mina === true) return false;
      if (refrigerio === "si" && r.refrigerio !== true) return false;
      if (refrigerio === "no" && r.refrigerio === true) return false;
      if (restaurante && normalizeText(r.restaurante) !== normalizeText(restaurante)) return false;

      const hasLunch = normalizeText(r.almuerzo).length > 0;
      if (almuerzo === "si" && !hasLunch) return false;
      if (almuerzo === "no" && hasLunch) return false;

      if (medioPago) {
        const abono = normalizeText(r.medio_abono);
        const pagoSaldo = normalizeText(r.medio_saldo);
        if (abono !== normalizeText(medioPago) && pagoSaldo !== normalizeText(medioPago)) return false;
      }
      if (saldo === "pendiente" && !(Number(r.saldo_pendiente) > 0)) return false;
      if (saldo === "pagado" && Number(r.saldo_pendiente) > 0) return false;

      if (q) {
        const haystack = [
          r.reserva_codigo, r.plan, r.fecha, formatDate(r.fecha), normalizeHour(r.hora), r.nombre, r.edad, r.documento,
          r.contacto, r.nacionalidad, r.observacion, r.restaurante, r.almuerzo, r.medio_abono, r.medio_saldo,
          yesNo(r.mina), yesNo(r.refrigerio), r.total, r.abono, r.pago_saldo, r.saldo_pendiente,
        ].map(normalizeText).join(" ");
        if (!haystack.includes(q)) return false;
      }
      return true;
    }).sort((a,b) => (b.id_reserva - a.id_reserva) || ((a.id_participante ?? 0) - (b.id_participante ?? 0)));
  }, [rows, search, fecha, plan, hora, mina, refrigerio, restaurante, almuerzo, medioPago, saldo]);

  const clearFilters = () => {
    setSearch(""); setFecha(""); setPlan(""); setHora(""); setMina(""); setRefrigerio("");
    setRestaurante(""); setAlmuerzo(""); setMedioPago(""); setSaldo("");
  };
  ...
  const saveEdit = async () => {
    ...
    try {
      await updateControlReserva(editing.id_reserva, {
        id_plan: editing.id_plan, id_hora: editing.id_hora, mina: editing.mina, refrigerio: editing.refrigerio,
        restaurante: editing.restaurante || null, valor_total: editing.total, valor_abonado: editing.abono,
        valor_saldo_pagado: editing.pago_saldo, metodo_pago_abono: editing.medio_abono || null,
        metodo_pago_saldo: editing.medio_saldo || null, observacion: editing.observacion || null,
      });
      if (editing.id_participante) {
        ...
        await updateControlParticipante(editing.id_participante, {
          nombre: editing.nombre || null, edad: editing.edad, nacionalidad: editing.nacionalidad || null,
          numero_documento: editing.documento || null, telefono_participante: participantPhone, tipo_almuerzo: editing.almuerzo.trim() || null,
        });
      }
      setEditing(null);
      await load(true);
  ...
  const filteredReservations = [...new Map(filtered.map(r => [r.id_reserva, r])).values()];
  ...
  return <div className="op-page">
    ...
    <div className="op-filters op-filters-complete">
      <div className="op-search"><Search size={16}/><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar código, nombre, documento, pago, observación…" /></div>
      ...
      <label><span>Horario</span><select value={hora} onChange={e => setHora(e.target.value)}><option value="">Todos</option>{horas.map(v => <option key={v} value={v}>{v}</option>)}</select></label>
      ...
      <label><span>Restaurante</span><select value={restaurante} onChange={e => setRestaurante(e.target.value)}><option value="">Todos</option>{restaurantesFiltro.map(v => <option key={v} value={v}>{v}</option>)}</select></label>
      ...
      <label><span>Medio de pago</span><select value={medioPago} onChange={e => setMedioPago(e.target.value)}><option value="">Todos</option>{metodosFiltro.map(v => <option key={v} value={v}>{v}</option>)}</select></label>
    ...
    <div className="op-summary"><span><Filter size={14}/> {filtered.length} filas</span><span>{filteredReservations.length} reservas</span><span>{filtered.filter(r => r.id_participante).length} participantes</span><span>Ventas: <b>{money(filteredReservations.reduce((s,r) => s+r.total,0))}</b></span><span>Saldo pendiente: <b>{money(filteredReservations.reduce((s,r) => s+r.saldo_pendiente,0))}</b></span></div>
    ...
    <div className="op-table-wrap"><table className="op-table"><thead><tr>
      ...
      {!filtered.length ? <tr><td colSpan={22} className="op-empty">No hay registros para los filtros seleccionados.</td></tr> : filtered.map((r,index) => {
        const isFirstOfReservation = previousReserva !== r.id_reserva;
        previousReserva = r.id_reserva;
        return <tr key={`${r.id_reserva}-${r.id_participante ?? index}`} className={isFirstOfReservation ? "group-start" : ""}>
          <td><div className="op-reserva"><strong>{r.reserva_codigo}</strong><span className="state ok">Aprobada</span></div></td>
          <td className="plan-cell">{r.plan || "—"}</td><td className="center">{formatDate(r.fecha)}</td><td className="center">{normalizeHour(r.hora)}</td>
    ...
    {editing && <div className="op-modal-backdrop"><div className="op-modal edit-modal"><div className="op-modal-head"><div><h2>Edición operativa</h2><p>Participante de la reserva {editing.reserva_codigo}</p></div><button onClick={() => setEditing(null)}><X size={20}/></button></div><div className="op-edit-grid">
      <label>Código de reserva<input value={editing.reserva_codigo} readOnly /></label><label>Fecha reserva<input value={formatDate(editing.fecha)} readOnly /></label>
      ...
      <label>Restaurante<select value={editing.restaurante || ""} onChange={e=>setEditing({...editing,restaurante:e.target.value})}><option value="">Sin restaurante</option>{editing.restaurante && !restaurantes.includes(editing.restaurante) && <option value={editing.restaurante}>{editing.restaurante} (histórico)</option>}{restaurantes.map(r=><option key={r} value={r}>{r}</option>)}</select></label>
      ...
      <label>Total<input type="number" value={editing.total} onChange={e=>setEditing({...editing,total:Number(e.target.value)})}/></label><label>Abono<input type="number" value={editing.abono} onChange={e=>setEditing({...editing,abono:Number(e.target.value)})}/></label>
      ...
      <label>Pago saldo<input type="number" value={editing.pago_saldo} onChange={e=>setEditing({...editing,pago_saldo:Number(e.target.value)})}/></label><label>Medio saldo<select value={editing.medio_saldo || ""} onChange={e=>setEditing({...editing,medio_saldo:e.target.value})}><option value="">Sin método</option>{editing.medio_saldo && !metodosPago.includes(editing.medio_saldo) && <option value={editing.medio_saldo}>{editing.medio_saldo} (histórico)</option>}{metodosPago.map(m=><option key={m} value={m}>{m}</option>)}</select></label>
