...
export default function ControlOperativoPage() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      const [operativo, medios, restaurantesData, pagosData, codigosData] = await Promise.all([
        getControlOperativo(), getMetodosPagoActivos(), getRestaurantesActivos(), getPagosControlOperativo(), getCodigosOperativos(),
      ]);
      setRows(operativo); setMetodos(medios); setRestaurantes(restaurantesData); setPagos(pagosData); setCodigos(codigosData);
  ...
  const filtered = useMemo(() => rows.filter((r) => {
    ...
    return !q || [r.reserva_codigo,r.plan,r.nombre,r.documento,r.contacto,r.observacion,r.medio_abono,r.medio_saldo].map(norm).join(" ").includes(q);
